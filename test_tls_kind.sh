...
#
# kind stands in for OpenShift: the overlay in workspace-tests/tests/tls_kind
...
# workspace-tests/tests/tls_kind.rs; this script only prepares the cluster.
#
...
kubectl apply --server-side --force-conflicts -k workspace-tests/tests/tls_kind
kubectl wait --for=condition=Established --timeout=60s \
    crd/apiservers.config.openshift.io \
    crd/modelexpressservers.modelexpress.opendatahub.io

echo "==> running workspace-tests/tests/tls_kind.rs"
cargo test -p model-express-workspace-tests --test tls_kind -- --ignored --nocapture ${TEST_ARGS[@]+"${TEST_ARGS[@]}"}
