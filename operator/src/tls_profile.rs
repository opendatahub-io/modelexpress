...
/// Fetch the cluster profile. `None` means the object or the whole API group
/// is absent, which is any non-OpenShift cluster. A 403 is returned as-is: the
...
pub async fn fetch(client: &Client) -> Result<Option<TlsProfile>, FetchError> {
    ...
    object
        .map(|obj| from_security_profile(&obj.data["spec"]["tlsSecurityProfile"]))
        .transpose()
...
mod tests {
    ...
    use crate::tls_profile::{
        ProfileError, TlsProfile, from_security_profile, needs_cluster_profile, resolve,
    };
    use serde_json::json;
