...
//! the cluster TLS profile, serving a service-ca certificate. Both are
//! reloaded in place, without a restart, when the certificate rotates or the
//! profile changes.
...
/// the certificate files change on disk (checked every `interval`; service-ca
/// rotates them in place) or `profiles` yields a new cluster profile.
///
...
where
    S: Stream<Item = TlsProfile> + Send + Unpin + 'static,
{
    let config = OpenSSLConfig::from_acceptor(Arc::new(acceptor(&paths, &profile)?));
    let reloading = config.clone();
    let mut reloader = Reloader::new(paths, profile);
    tokio::spawn(async move {
        let mut profiles = profiles.fuse();
        let mut ticker = tokio::time::interval(interval);
        loop {
            let trigger = tokio::select! {
                _ = ticker.tick() => Trigger::CertCheck,
                Some(profile) = profiles.next() => Trigger::Profile(profile),
            };
            match reloader.rebuild(trigger) {
                ...
                Some(Ok(acceptor)) => {
                    ...
                    tracing::info!(
                        min_version = %reloader.profile.min_version,
...
struct Reloader {
    paths: CertPaths,
    /// The profile the listener should serve, even when building it failed.
    profile: TlsProfile,
...
impl Reloader {
    fn new(paths: CertPaths, profile: TlsProfile) -> Self {
    ...
    fn rebuild(&mut self, trigger: Trigger) -> Option<Result<SslAcceptor, MetricsTlsError>> {
        ...
        match trigger {
            ...
            Trigger::CertCheck => {}
            Trigger::Profile(profile) if profile == self.profile => return None,
            Trigger::Profile(profile) => self.profile = profile,
        }
        let rebuilt = acceptor(&self.paths, &self.profile);
        if rebuilt.is_ok() {
...
pub(crate) mod tests {
    ...
    fn broken_profile() -> TlsProfile {
        TlsProfile {
            min_version: "TLS9".to_string(),
    ...
    fn reloader_ignores_unchanged_inputs() {
        let dir = TempDir::new().expect("tempdir");
        let mut reloader = Reloader::new(self_signed(dir.path()), TlsProfile::intermediate());
        assert!(reloader.rebuild(Trigger::CertCheck).is_none());
        assert!(
            reloader
                .rebuild(Trigger::Profile(TlsProfile::intermediate()))
                .is_none()
        );
    }

    #[test]
    fn reloader_rebuilds_once_per_profile_change() {
        let dir = TempDir::new().expect("tempdir");
        let mut reloader = Reloader::new(self_signed(dir.path()), TlsProfile::intermediate());
        let rebuilt = reloader
            .rebuild(Trigger::Profile(TlsProfile::modern()))
    ...
    #[test]
    fn reloader_does_not_retry_a_profile_that_failed_to_build() {
        let dir = TempDir::new().expect("tempdir");
        let mut reloader = Reloader::new(self_signed(dir.path()), TlsProfile::intermediate());
        assert!(matches!(
            reloader.rebuild(Trigger::Profile(broken_profile())),
            Some(Err(_))
        ));
        assert!(
            reloader
                .rebuild(Trigger::Profile(broken_profile()))
                .is_none()
        );
        assert!(reloader.rebuild(Trigger::CertCheck).is_none());
        assert!(matches!(
            reloader.rebuild(Trigger::Profile(TlsProfile::intermediate())),
            Some(Ok(_))
        ));
    }
    ...
    fn reloader_retries_a_half_rotated_key_pair() {
        let dir = TempDir::new().expect("tempdir");
        let paths = self_signed(dir.path());
        let mut reloader = Reloader::new(paths.clone(), TlsProfile::intermediate());
    ...
    async fn profile_change_applies_to_new_handshakes_without_a_restart() {
        let dir = TempDir::new().expect("tempdir");
        ...
        tx.send(TlsProfile::modern()).await.expect("send profile");
