...
async fn metrics_over_tls(
    ...
    use modelexpress_operator::metrics_tls::{CertPaths, RELOAD_INTERVAL, config_with_reload};
    ...
    let updates = tls_profile::profile_updates(client.clone(), profile.clone()).await;
    let config = config_with_reload(CertPaths::in_dir(&dir), profile, updates, RELOAD_INTERVAL)?;
