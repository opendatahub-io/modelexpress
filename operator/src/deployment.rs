...
use crate::tls_profile::{MOUNT_PATH, ResolvedTls, TlsProfile, resolve};
...
pub const SERVING_CERT_ANNOTATION: &str = "service.beta.openshift.io/serving-cert-secret-name";
...
/// `cluster_tls` is the profile the cluster is on; it only matters when
/// `spec.tls` is set and leaves version or ciphers to the cluster.
pub fn render(
    cr_name: &str,
    spec: &ModelExpressServerSpec,
    cluster_tls: &TlsProfile,
) -> DesiredState {
    ...
    let tls = spec.tls.as_ref().map(|config| resolve(config, cluster_tls));
    ...
    let service = Service {
        metadata: ObjectMeta {
            name: Some(cr_name.to_string()),
            labels: Some(labels),
            annotations: service_annotations(tls.as_ref()),
            ..ObjectMeta::default()
        },
...
/// The service-ca annotation when the CR asked OpenShift to issue the cert.
fn service_annotations(tls: Option<&ResolvedTls>) -> Option<BTreeMap<String, String>> {
    let tls = tls.filter(|tls| tls.service_ca)?;
    Some(BTreeMap::from([(
        SERVING_CERT_ANNOTATION.to_string(),
...
mod tests {
    ...
    fn selector_matches_pod_labels() {
        let state = render("mx", &base_spec(), &TlsProfile::default());
    ...
    fn grpc_probes_target_the_server_port() {
        ...
        spec.port = 9000;
        let state = render("mx", &spec, &TlsProfile::default());
        let c = container(&state);
    ...
    fn ephemeral_cache_rolls_but_pvc_cache_recreates() {
        let strategy = |spec: &ModelExpressServerSpec| {
            render("mx", spec, &TlsProfile::default())
                .deployment
                .spec
                .expect("spec")
                .strategy
    ...
    fn network_policy_absent_by_default() {
        assert!(
            render("mx", &base_spec(), &TlsProfile::default())
                .network_policy
                .is_none()
        );
    }
    ...
    fn network_policy_restricts_grpc_port_to_peers() {
        ...
        let netpol = render("mx", &spec, &TlsProfile::default())
            .network_policy
            .expect("netpol");
