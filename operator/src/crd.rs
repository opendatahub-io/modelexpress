...
/// Serve TLS from a kubernetes.io/tls Secret. Version, ciphers and groups
/// follow the cluster profile (apiservers.config.openshift.io/cluster) when its
/// tlsAdherence is StrictAllComponents, and the Intermediate profile otherwise,
/// unless pinned here. Kubernetes gRPC probes cannot speak TLS, so the probes
/// switch to TCP when this is set.
#[derive(CELSchema, Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct TlsConfig {
    ...
    /// Annotate the Service so OpenShift's service-ca issues secretName.
    #[serde(default)]
    pub service_ca: bool,
